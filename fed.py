...
def create_mask(img,number):
    ...
    return cv2.adaptiveThreshold(img,255,cv2.ADAPTIVE_THRESH_GAUSSIAN_C,cv2.THRESH_BINARY,17,3.5)


def largest_contour(mask):
    contours = cv2.findContours(mask, cv2.RETR_LIST, cv2.CHAIN_APPROX_TC89_L1) #cv.CHAIN_APPROX_SIMPLE,CHAIN_APPROX_TC89_KCOS
